  ...
  for(k in 1:Stages_Interval){
    if(!is.na(Diag[i,k])){
      ...
      if(Diag[i,k]==1 & C_Diag[i,k]==0){
        delay_original[i,k]=Dur[i,k]+Waiting_time[i,k]+max(Delta_diffe_wrong[i,k],0)
      }
    }
  }
}
...
diagnostic_delay_original<-as.numeric(TB_Pat_Pathway$care_diag)
...
dur<-array(dim=c(simulations,stages))
   ...
   for(j in 1:simulations){
    ...
    sim_TPM[4,]=para[29:32]
      ...
      if(cat[j,1]!=3){
        d_dur[j,1]<-rexp(1,rate=sim_mu_d[cat[j,1]]+0.0000000000000000000000000000000000000001)
      }else{
        d_dur[j,1]=999999999999999999999999999999999
      }
      s_dur[j,1]<-rexp(1,rate=sim_mu_s[cat[j,1]]+0.000000000000000000000000000000000000000000001)
      dur[j,1]<-min(d_dur[j,1],s_dur[j,1],500)
      ...
      if(cdiag[j,1]==0){
        ...
        if(cat[j,2]!=3){
          d_dur[j,2]<-rexp(1,rate=sim_mu_d[cat[j,2]]+0.0000000000000000000000000000000000000001)
        }else{
          d_dur[j,2]=999999999999999999999999999999999
        }
        s_dur[j,2]<-rexp(1,rate=sim_mu_s[cat[j,2]]+0.000000000000000000000000000000000000000000001)
        dur[j,2]<-min(d_dur[j,2],s_dur[j,2],500)
        ...
        if(cdiag[j,2]==0){
          ...
          if(cat[j,3]!=3){
            d_dur[j,3]<-rexp(1,rate=sim_mu_d[cat[j,3]]+0.0000000000000000000000000000000000000000000000000000000001)
          }else{
            d_dur[j,3]<-99999999999999999999999999999999
          }
          s_dur[j,3]<-rexp(1,rate=sim_mu_s[cat[j,3]]+0.0000000000000000000000000000000000000000000000000000001)
          dur[j,3]<-min(d_dur[j,3],s_dur[j,3],500)
          ...
          if(cdiag[j,3]==0){
            ...
            if(cat[j,4]!=3){
              d_dur[j,4]<-rexp(1,rate=sim_mu_d[cat[j,4]]+0.0000000000000000000000000000000000000000000001)
            }else{
              d_dur[j,4]<-999999999999999999999999999999
            }
            s_dur[j,4]<-rexp(1,rate=sim_mu_s[cat[j,4]]+0.0000000000000000000000000000000000000001)
            dur[j,4]<-min(d_dur[j,4],s_dur[j,4],500)
    ...
    mean(diagnostic_delay_sim)
    mean(diagnostic_delay_original)
